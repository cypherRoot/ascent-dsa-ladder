...
Climb the patterns that actually get asked. A curated ladder of **173 problems**
(NeetCode 150 + Striver-SDE high-frequency set + a thin Java warmup tier),
...
- **Two views, one dataset** — `Ladder.tsx` just groups `problems.json` by
  `topic` or by `pattern`. Because progress keys on `id`, solving in one view
